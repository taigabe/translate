class TranslateController < ActionController::Base
  ...
  def force_init_translations
    I18n.load_path = (I18n.load_path + Dir.glob(File.join("config", "locales", "**","*.{rb,yml}"))).uniq.sort
  ...
  def set_locale
    @from_locale = params[:from_locale].present? ? params[:from_locale] : default_locale
    @to_locale   = params[:to_locale].present?   ? params[:to_locale]   : :en
  end
